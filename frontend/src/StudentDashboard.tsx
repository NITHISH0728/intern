            ...
            <div className="flex flex-col lg:flex-row h-screen bg-[#F8FAFC] font-sans overflow-hidden relative">
                ...
                <div className="w-full lg:w-[65%] h-[60%] lg:h-full flex flex-col bg-[#F3F4F6]">
                    <div className="h-10 lg:h-12 bg-white border-b border-slate-200 flex items-center justify-between px-2 lg:px-4 shrink-0">
                        ...
                        <select value={language} onChange={(e) => setLanguage(Number(e.target.value))} className="text-[10px] lg:text-xs border border-slate-300 rounded px-2 py-1 bg-white font-bold text-slate-700">
                    ...
                    <div className="h-14 lg:h-16 bg-white border-t border-slate-200 flex items-center justify-end px-4 lg:px-6 gap-2 lg:gap-4 shrink-0">
                        <button onClick={() => switchQuestion(currentProblemIndex + 1 < activeTest.problems.length ? currentProblemIndex + 1 : 0)} className="flex items-center gap-2 px-4 py-2 lg:px-6 lg:py-2.5 rounded-lg border border-slate-300 text-slate-700 font-bold text-xs lg:text-sm hover:bg-slate-50 transition-colors"><Save size={14} className="lg:w-4 lg:h-4" /> <span className="hidden sm:inline">Save & Next</span><span className="sm:hidden">Next</span></button>
                        <button onClick={handleRun} disabled={executionStatus === "running"} className="flex items-center gap-2 px-4 py-2 lg:px-8 lg:py-2.5 rounded-lg bg-[#005EB8] text-white font-bold text-xs lg:text-sm hover:bg-blue-700 shadow-md transition-all"><Play size={14} fill="currentColor" className="lg:w-4 lg:h-4" /> Run Code</button>
                    </div>
                </div>
